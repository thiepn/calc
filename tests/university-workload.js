...
approx(N.deserializeValue(doc.blocks[3].result.serialized),2.5,1e-12,"U6 numeric block reference value");
assert(doc.blocks[3].result.metadata.exact===false,"U6 numeric block reference preserves approximate provenance");
eq(doc.blocks[3].result.serialized.type,"primitive","U6 downstream reference stays a real primitive");
assert(!doc.blocks[3].result.display.includes("/2500000000000000"),"U6 approximate reference is not relabeled as a huge exact rational");
