...
function parseRelationPairs(universe,source){
  ...
  splitArgs(source).forEach(function(item){var idx=item.indexOf(">");if(idx<1||idx===item.length-1)throw new DiscreteMathError("RELATION_PARSE_ERROR","Relation pairs use a>b syntax",{pair:item});var a=normalizeAtom(item.slice(0,idx)),b=normalizeAtom(item.slice(idx+1));if(!allowed.has(a)||!allowed.has(b))throw new Dis