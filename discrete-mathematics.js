...
function pruferDecode(code){var seq=code.map(function(x){return toBoundedInt(x,1,LIMITS.combinatorialN,"Prüfer label");}),n=seq.length+2;if(n>LIMITS.combinatorialN)throw new DiscreteComplexityError("Prüfer tree size exceeds safety limit");var degree=Array(n+1).fill(1);seq.forEach(function(x){if(x>n)throw new DiscreteMathError("PRUFER_LABEL","Prüfer labels must be in 1..n",{label:x,n:n});degree[x]++;});var edges=[];seq.forEach(function(x){var leaf=1;while(leaf<=n&&degree[leaf]!==1)leaf++;edges.push([String(leaf),String(x)]);degree[leaf]--;degree[x]--;});var leaves=[];f